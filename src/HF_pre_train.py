...
class DataTrainingArguments:
    ...
    soft_masked_loss_weights_train: Optional[float] = field(default=1.0)
    soft_masked_loss_weights_evaluation: Optional[float] = field(default=1.0)
    soft_masked_loss_weights_test: Optional[float] = field(default=1.0)
...
def main():
    ...
    if model_args.config_name:
        config = AutoConfig.from_pretrained(model_args.config_name,trust_remote_code=True, dtype=torch.bfloat16, **config_kwargs)
    elif model_args.model_name_or_path:
        config = AutoConfig.from_pretrained(
            model_args.model_name_or_path, trust_remote_code=True, dtype=torch.bfloat16, **config_kwargs
        )
    else:
    ...
    def tokenize_function(examples, soft_masked_weight):
        ...
        res["loss_weights"] = np.ones_like(res["input_ids"], dtype=float)
        res["loss_weights"][
    ...
    soft_masked_weight = {
        "train": data_args.soft_masked_loss_weights_train,
        "validation": data_args.soft_masked_loss_weights_evaluation,
    }
    ...
    if training_args.do_train:
        train_dataset = raw_datasets["train"].map(
            lambda examples: tokenize_function(
                examples, data_args.soft_masked_loss_weights_train
            ),
            batched=True,
    ...
    if training_args.do_eval:
        eval_dataset = raw_datasets["validation"].map(
            lambda examples: tokenize_function(
                examples, data_args.soft_masked_loss_weights_evaluation
            ),
            batched=True,
            remove_columns=remove_columns,
        )
    ...
    if data_args.do_test:
        test_dataset = raw_datasets["test"].map(
            lambda examples: tokenize_function(
                examples, data_args.soft_masked_loss_weights_test
            ),
            batched=True,
            remove_columns=remove_columns,
        )
